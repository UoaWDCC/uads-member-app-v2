  ...
  return (
    <>
      <div
        ...
        data-testid="socialsComponent"
      >
        {socials.slice(0, -1).map((social, index) => (
          <div
            key={index}
            className={`w-10 h-10 ${background} rounded-full hover:${hoverBackground} cursor-pointer group`}
          >
            <Link
              to={
                social.name === "Email" ? `mailto:${social.link}` : social.link
              }
              target="_blank"

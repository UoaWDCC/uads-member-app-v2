		...
		<div className="max-w-screen h-24 bg-pink relative px-10">
				...
				<div className="w-full h-auto flex justify-between items-center select-none">
					...
					<div className="hidden lg:flex justify-between text-2xl">
						{navlinks.map((link, index) => (
							<NavLink key={index} className={({ isActive }) => `font-raleway px-10 transition-all duration-300 font-bold ${isActive ? "text-yellow" : "text-light-pink  hover:text-yellow "}`} to={link.link}>
								{link.title}
							</NavLink>
						))}
					</div>
			...
			{open && (
				<div className="lg:hidden absolute top-0 right-0 w-full bg-pink pt-20 pb-4 z-10 flex flex-col items-center">
					...
					{navlinks.map((link, index) => (
						<NavLink key={index} className={({ isActive }) => `block py-4 bg-pink font-raleway transition-all duration-300 font-bold ${isActive ? "text-yellow" : "text-light-pink  hover:text-yellow "}`} to={link.link} onClick={() => setOpen(false)}>
							{link.title}
						</NavLink>
					))}
					<NavLink className={({ isActive }) => `block py-4 bg-pink font-raleway transition-all duration-300 font-bold ${isActive ? "text-yellow" : "text-light-pink  hover:text-yellow "}`} to={"/signup"} onClick={() => setOpen(false)}>
						Join
					</NavLink>
				</div>
			)}
